...
Write a handoff document summarising the current conversation so a fresh agent can continue the work. If currently on a branch that has a ticket id, save to `.agents/<ticket-id>/` directory - otherwise save to `.agents/<new-folder-name>` directory, where the folder name is based on the details of the interaction. Keep that folder name brief. 
